...
{                                                                               \
    ...
    {                                                                           \
        MT(x)->old = MT(x)->end;                                              \
        MT(x)->end += (off_t)MTREE_INCRE_NUM * (off_t)sizeof(MTNODE);       \
        if(MT(x)->old == sizeof(MTSTATE))                                  \
        {                                                                   \
            memset(MT(x)->state, 0, sizeof(MTSTATE));                      \
            MT(x)->state->left += MTREE_INCRE_NUM - 1;                  \
        }                                                                   \
        else                                                                \
        {                                                                   \
            MT(x)->state->left += MTREE_INCRE_NUM;                      \
        }                                                                   \
        MT(x)->state->total += MTREE_INCRE_NUM;                         \
...
void mtree_mutex_lock(void *x, int id)
{
    if(x)
    {
#ifdef HAVE_PTHREAD
        pthread_mutex_lock(&(MT(x)->mutexs[id%MTREE_MUTEX_MAX]));
#endif
    }
    return ;
}
void mtree_mutex_unlock(void *x, int id)
{
    if(x)
    {
#ifdef HAVE_PTHREAD
        pthread_mutex_unlock(&(MT(x)->mutexs[id%MTREE_MUTEX_MAX]));
#endif
    }
    return ;
}
...
void *mtree_init()
{
    ...
    if((x = (MTREE *)xmm_mnew(sizeof(MTREE))))
    {
        ...
        MT(x)->size = (off_t)sizeof(MTSTATE) + (off_t)sizeof(MTNODE) * (off_t)MTREE_NODES_MAX;
        //mmap
        MT_MAP(x);
        MT(x)->end = (off_t)sizeof(MTSTATE);
        MT_INCRE(x);
        /* initialize mutexs  */
#ifdef HAVE_PTHREAD
        for(i = 0; i < MTREE_MUTEX_MAX; i++)
        {
...
int mtree_new_tree(void *x)
{
    ...
    if(x)
    {
        ...
        if(MT(x)->state && MT(x)->state->nroots < MTREE_ROOT_MAX)
        {
            for(i = 1; i < MTREE_ROOT_MAX; i++)
            {
...
/* total */
unsigned int mtree_total(void *x, int rootid)
{
...
//add nodeid to qleft
void mtree_qleft(void *x, int tnodeid)
{
    int z = 0;
    if(x)
    {
...
//new node
unsigned int mtree_new_node(void *x, int rootid, int nodeid, int key, int data)
{
    unsigned int id = 0;
...
/* insert new node */
unsigned int mtree_insert(void *x, int rootid, int key, int data, int *old)
{
    ...
    if(x && rootid > 0)
    {
                ...
                && MT(x)->state->roots[rootid].status > 0)
        {
            ...
            //new node
            if(id == 0) id = mtree_new_node(x, rootid, nodeid, key, data);
        }
        if((nodeid = id) > 0)
        {
...
/* try insert  node */
unsigned int mtree_try_insert(void *x, int rootid, int key, int data, int *old)
{
    unsigned int id = 0, nodeid = 0, rid = 0, lid = 0, uid = 0, pid = 0, 
        gpid = 0, ppid = 0, *prootid = NULL;
    MTNODE *node = NULL;
...
/* get node key/data */
unsigned int mtree_get(void *x, unsigned int tnodeid, int *key, int *data)
{
    unsigned int id = 0;
...
/* find key/data */
unsigned int mtree_find(void *x, int rootid, int key, int *data)
{
    unsigned int id = 0;
...
unsigned int mtree_min(void *x, int rootid, int *key, int *data)
{
    unsigned int id = 0;
...
unsigned  int mtree_max(void *x, int rootid, int *key, int *data)
{
...
unsigned int mtree_next(void *x, int rootid, unsigned int tnodeid, int *key, int *data)
{
...
unsigned int mtree_prev(void *x, int rootid, unsigned int tnodeid, int *key, int *data)
{
    unsigned int id = 0, parentid = 0;
...
/* view node */
void mtree_view_tnode(void *x, unsigned int tnodeid, FILE *fp)
{
    if(x)
    {
    ...
    if(x && rootid > 0)
    {
        ...
        if(MT(x)->map && MT(x)->state && rootid < MTREE_ROOT_MAX)
        {
            fprintf(stdout, "%s::%d rootid:%d\n", __FILE__, __LINE__, MT(x)->state->roots[rootid].rootid);
             mtree_view_tnode(x, MT(x)->state->roots[rootid].rootid, fp);
        }
        mtree_mutex_unlock(x, rootid);
    }
    return ;
...
/* set data */
int mtree_set_data(void *x, unsigned int tnodeid, int data)
{
    int old = -1;
...
/* remove node */
void mtree_remove(void *x, int rootid, unsigned int tnodeid, int *key, int *data)
{
...
{
    if(x && rootid > 0 && rootid < MTREE_ROOT_MAX)
    {
        mtree_mutex_lock(x, rootid);
        mtree_remove_tnode(x, MT(x)->state->roots[rootid].rootid);
...
void mtree_reuse_all(void *x)
{
    ...
    if(x)
    {
        ...
        for(i = 1; i < MTREE_ROOT_MAX; i++)
        {
            mtree_remove_tnode(x, MT(x)->state->roots[i].rootid);
            MT(x)->state->roots[i].rootid = 0;
            MT(x)->state->roots[i].status = 1;
            MT(x)->state->nroots++;
        }
    }
...
void mtree_close(void *x)
{
    ...
    if(x)
    {
        //fprintf(stdout, "%s::%d start:%p state:%p map:%p current:%d left:%d total:%d qleft:%d qfirst:%d qlast:%d sizeof(MTSTATE):%d\n", __FILE__, __LINE__, MT(x)->start, MT(x)->state, MT(x)->map, MT(x)->state->current, MT(x)->state->left, MT(x)->state->total, MT(x)->state->qleft, MT(x)->state->qfirst, MT(x)->state->qlast, sizeof(MTSTATE));
        MT_MUNMAP(x);
...
int main(int argc, char **argv) 
{
    ...
    if((mtree = mtree_init("/tmp/test.mtree")))
    {
        ...
        for(j = 1; j <= count; j++)
        {
            ...
            old = -1;
            data = j;
            id = mtree_insert(mtree, rootid, key, data, &old);
            if(old > 0 || id <= 0) 
            {
        ...
        for(j = 1; j <= count; j++)
        {
            ...
            old = -1;
            data = j;
            id = mtree_try_insert(mtree, rootid, key, data, &old);
            if(old > 0 && old != j) 
            {
                fprintf(stdout, "%d:{id:%d key:%d rootid:%d old:%d}\n", j, id, key, rootid, old);
                _exit(-1);
...
//gcc -o mtree mtree.c md5.c -D_DEBUG_MTREE -g && ./mtree
#endif
